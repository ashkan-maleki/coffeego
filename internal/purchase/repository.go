...
func NewMongoRepository(ctx context.Context, connectionString string) (*MongoRepository, error) {
	...
	purchases := client.Database("coffeego").Collection("purchases")
...
func (m *MongoRepository) Store(ctx context.Context, purchase Purchase) error {
	...
	_, err := m.purchases.InsertOne(ctx, mongoP)
	if err != nil {
...
type mongoPurchase struct {
	...
	ProductsToPurchase []coffeego.Product `bson:"products_purchased"`
